...
def solvated_path(input: MinimizationInput) -> Path:
    return input.directory / "solvated.gro"


def corrected_box_path(input: MinimizationInput) -> Path:
...
def get_topname(input: MinimizationInput, triple: ComputationTriple) -> TopName:
    return {
...
def solvate(ctx: Context, input: MinimizationInput, triple: ComputationTriple):
...
def add_ions(ctx: Context, input: MinimizationInput, triple: ComputationTriple):
...
def energy_minimize(ctx: Context, input: MinimizationInput, triple: ComputationTriple):
...
def editconf_task(ctx: Context, input_ligand: MinimizationInput, input_protein: MinimizationInput):
...
def energy_minimization_task(ctx: Context, input: MinimizationInput, triple: ComputationTriple):
    solvate(ctx, input, triple)
    add_ions(ctx, input, triple)
    energy_minimize(ctx, input, triple)


def solvate_prepare(ctx: Context, triple: ComputationTriple, job: Job) -> List[Task]:
    ...
    inputs = [
        MinimizationInput(lop=LigandOrProtein.Ligand, directory=ligand_dir),
        MinimizationInput(lop=LigandOrProtein.Protein, directory=protein_dir)
    ]
    task = job.function(editconf_task, args=(ctx, inputs[0], inputs[1]))
    return [job.function(energy_minimization_task, args=(ctx, input, triple), deps=[task],
