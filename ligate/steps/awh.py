...
def awh_task(ctx: Context, lop: LigandOrProtein, workload: EquilibratePartOutput,
             directory: Path, mdp_path: Path, triple: ComputationTriple):
...
def awh(ctx: Context, triple: ComputationTriple, params: AWHParams,
        equilibrate_output: EquilibrateOutput, job: Job) -> AWHOutput:
    ...
    def create_tasks(lop: LigandOrProtein, output: EquilibratePartOutput, dependency: Task) -> \
        ...
        for run in range(1, params.replicates + 1):
            ...
            tasks.append(AWHPartOutputWithTask(
                task=task,
                output=AWHPartOutput(
                    lop=lop,
                    awh_directory=awh_directory,
                    run_directory=run_dir
                )
            ))
        return tasks

    outputs = create_tasks(LigandOrProtein.Ligand, equilibrate_output.ligand_output,
                           equilibrate_output.ligand_task)
    outputs += create_tasks(LigandOrProtein.Protein, equilibrate_output.protein_output,
