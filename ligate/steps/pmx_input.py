...
class PmxInputProvider(InputProvider):
    ...
    def provide_input(self, input: ComputationTriple, directory: Path):
        ...
        structure_protein_files = paths_in_dir(structure_protein_filenames(input), proteinff_dir)
        copy_files(structure_protein_files, structure_dir)

        transformation_dir = protein_dir / f"transformations_{ligand_ff(input)}" / mutation_name(
            input.mutation)
        structure_ligand_files = paths_in_dir(STRUCTURE_LIGAND_FILENAMES, transformation_dir)
        copy_files(structure_ligand_files, structure_dir)

        # Fuse all elements into one PDB file
        merged_pdb = structure_dir / "full.pdb"
        append_lines_to(get_file_lines(structure_protein_files[0]), merged_pdb, "ENDMDL")
        append_lines_to(get_file_lines(structure_dir / "mergedA.pdb", skip=2),
                        merged_pdb, "ENDMDL")

        assert len(structure_protein_files) <= 3
        if len(structure_protein_files) > 2:
            append_lines_to(get_file_lines(structure_protein_files[2]), merged_pdb, "ENDMDL")
        ...
        copy_files(paths_in_dir(TOPOLOGY_LIGAND_FILENAMES, transformation_dir), topology_dir)
        ...
        replace_in_place(topology_dir / f"topol_{protein_ff(input)}.top",
                         [(f"{get_ff_name(input)}.ff", ff_path)])

        append_to(topology_dir / "topol_ligandInWater.top", f"""; Include forcefield parameters
...
def get_ff_name(input: ComputationTriple) -> str:
    return {
        ForceField.Amber: "amber99sb-star-ildn-mut"
    }[input.forcefield]
...
def get_ff_path(input: ComputationTriple) -> str:
    ...
    """
    return {
        ForceField.Amber: f"pmx/data/mutff45/{get_ff_name(input)}.ff"
    }[input.forcefield]


def get_topology_chain_1(input: ComputationTriple) -> str:
    return {
        Protein.Bace: "topol.itp"
    }[input.protein]
